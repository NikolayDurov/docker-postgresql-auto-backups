...
psql -h localhost -U "$LOGIN" -d postgres -Atc "SELECT datname FROM pg_database WHERE datistemplate = false" | while read -r DBNAME; do
    BACKUP_FILE="$BACKUP_DIR${DBNAME}_${TIMESTAMP}.tar"
    pg_dump --no-owner --no-acl --dbname="postgresql://${POSTGRES_USER}:${POSTGRES_PASSWORD}@localhost:5432/${DBNAME}" -Fc -o > "$BACKUP_FILE"
    if [ $? -eq 0 ]; then
